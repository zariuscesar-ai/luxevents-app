...
import { Star, MapPin, Globe, Instagram, CheckCircle, Users, Clock } from 'lucide-react'
import TrackView from '@/components/provider/TrackView'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: data.business_name + ' — LuxEvents', description: data.description?.slice(0, 160) }
}
      ...
      <div className="pt-16">
        <div className="relative h-72 md:h-96 bg-gray-800">
          {p.cover_image_url ? (
            ...
            <Image src={images[0].url} alt={p.business_name} fill className="object-cover" />
          ) : (
          ...
          <div className="relative -mt-16 mb-6 flex flex-col md:flex-row md:items-end gap-4">
            <div className="w-28 h-28 rounded-2xl border-4 border-white shadow-xl overflow-hidden bg-white flex-shrink-0">
              {p.logo_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-4xl bg-yellow-50">{CATEGORY_ICONS[p.category]}</div>
              )}
            </div>
              ...
              <div className="flex flex-wrap items-start gap-3">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                    <span className="inline-flex items-center gap-1 bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-full font-medium text-xs">{CATEGORY_ICONS[p.category]} {CATEGORY_LABELS[p.category]}</span>
                    {p.city && <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {p.city}, {p.state}</span>}
                    {p.review_count > 0 && <span className="flex items-center gap-1"><Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />{p.rating.toFixed(1)} ({p.review_count} reviews)</span>}
                  </div>
                </div>
                <div className="md:ml-auto flex gap-2 flex-wrap">
                  {p.website && <a href={p.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 border border-gray-200 text-gray-600 hover:border-yellow-400 hover:text-yellow-600 px-3 py-1.5 rounded-lg text-sm transition-colors"><Globe className="w-4 h-4" /> Website</a>}
                  {p.instagram && <a href={'https://instagram.com/' + p.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 border border-gray-200 text-gray-600 hover:border-yellow-400 hover:text-yellow-600 px-3 py-1.5 rounded-lg text-sm transition-colors"><Instagram className="w-4 h-4" /> Instagram</a>}
                </div>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">            <div className="lg:col-span-2 space-y-8">
              {p.description && (
              ...
              {p.services && p.services.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3">
                    {p.services.filter((s: any) => s.is_active).map((service: any) => (
                      <div key={service.id} className="flex items-start justify-between p-4 border border-gray-100 rounded-xl hover:border-yellow-200 hover:bg-yellow-50/30 transition-colors">
                        ...
                        <div className="text-right flex-shrink-0 ml-4">
                          {service.price ? <span className="font-bold text-gray-900">${service.price.toLocaleString()}</span> : <span className="text-sm text-gray-400">Quote on request</span>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-4">
                    {reviews.map((review: any) => (
                      <div key={review.id} className="border-b border-gray-50 last:border-0 pb-4 last:pb-0">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center text-sm font-medium text-yellow-700">{review.client?.full_name?.[0] ?? 'C'}</div>
                          <div>
